...
def fetch_price_data(tickers: list, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    try:
        ...
        # Correção para atualização recente do yfinance que retorna MultiIndex
...
def fetch_fundamentals(tickers: list) -> pd.DataFrame:
    ...
    data = []
    # Remove SPY da lista de fundamentais se estiver lá apenas para cálculo de preço
...
def compute_fundamental_momentum(fund_df: pd.DataFrame) -> pd.Series:
    ...
    for m in metrics:
        if m in fund_df.columns:
            # Preenche NaN com mediana para não penalizar demais
...
def compute_value_score(fund_df: pd.DataFrame) -> pd.Series:
    ...
    if 'forwardPE' in fund_df:
        # Se PE > 0, Yield = 1/PE. Se PE < 0 (prejuízo), yield = 0
...
def run_backtest(weights: pd.Series, prices: pd.DataFrame, lookback_days: int = 252):
    """
    Simula o desempenho do portfólio selecionado no período recente (In-Sample Analysis).
    """
    ...
    if 'SPY' in prices.columns:
        ...
        # Alinha datas
        combined = pd.DataFrame({'Strategy': cumulative, 'SPY': spy_cum}).dropna()
        return combined
...
def main():
    ...
    target_vol = 0.15 if use_vol_target else None
    ...
    if run_btn:
        ...
        with st.status("Executando Pipeline Quant...", expanded=True) as status:
            ...
            start_date = end_date - timedelta(days=730) # 2 anos para regressão sólida
            ...
            # Remove quem não tem dados suficientes
        ...
        with tab2:
            ...
            st.info("Nota: Este backtest simula como a carteira SELECIONADA HOJE teria performado nos últimos 12 meses (In-Sample Analysis). Não é um backtest de 'walk-forward'.")
        ...
        with tab3:
            ...
            st.subheader("Dados Brutos (Fundamentos)")
